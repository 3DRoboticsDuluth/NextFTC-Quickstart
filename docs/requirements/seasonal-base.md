...
| `REQ-SCF-002` | Empty `Teleop` and `Auto` classes MUST be FTC-discoverable and compile without robot mechanisms. | A new season needs a deployable starting point from day one. | `TeamCode:assembleDebug` passes on Quickstart `main`. |
...
| `REQ-SCF-004` | The neutral config MUST include shared diagnostic level and filter behavior without season selections. | Diagnostics are platform behavior; alliance, side, and strategy are game behavior. | Only reusable diagnostics settings exist on Quickstart `main`. |
