...
class DeferredCommandTests {
    ...
    fun delegatedFactoriesCreateFreshNamedCommandsWithArguments() {
        ...
        assertEquals(arrayOf(TestSubsystem).toList(), TestSubsystem.one.requirements.toList())
        assertTrue(TestSubsystem.one.command(0) is LambdaCommand)
        assertTrue(TestSubsystem.three.command(0, 0, 0) is LambdaCommand)
        assertEquals(2, TestSubsystem.defaulted.second)
        assertEquals(3, TestSubsystem.defaulted.third)
        assertTrue(TestSubsystem.defaulted.command(0, 0, 0) is LambdaCommand)
        TestSubsystem.arguments = emptyList()
